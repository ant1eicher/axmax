...
impl QPIGS {
    pub fn new_from_string(input_data_string: &str) -> Result<Self> {
        ...
        if parts.len() < 16 {
            bail!("Insufficient data in QPIGS response")
        }
        ...
        Ok(QPIGS {
            ...
            pv1_input_current: parts[12].parse::<f32>()?,
            pv1_input_voltage: parts[13].parse::<f32>()?,
            battery_voltage_from_scc: parts[14].parse::<f32>()?,
            battery_discharge_current: parts[15].parse::<usize>()?,
            pv1_charging_power: parts[19].parse::<usize>()?,
        })
    }
}
